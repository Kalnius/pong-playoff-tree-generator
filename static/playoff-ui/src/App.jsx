...
function getParentOrigin() {
  try {
    return window.parent !== window && document.referrer ? new URL(document.referrer).origin : window.location.origin;
  } catch {
...
function postStateToParent(payload) {
  if (window.parent && window.parent !== window) {
    window.parent.postMessage({ type: "playoff:state", payload }, getParentOrigin());
  }
}
...
export default function App() {
  ...
  useEffect(() => {
    const onMessage = (event) => {
      ...
      if (msg.type === "playoff:request-state") {
        event.source?.postMessage({ type: "playoff:state", payload }, getParentOrigin());
      }
  ...
  const onCopyPng = async () => {
    ...
    try {
      ...
      setTemporaryMessage(setPngMsg, "Clipboard access is not available in this browser.");
    }
  };
    ...
    <div className={`container${embedMode ? " embed-mode" : ""}`}>
      ...
      {!embedMode && (
        <div className="info-panel">
          <strong>Important GitHub Pages persistence note:</strong> local browser save is still available, but the durable,
          redeploy-safe version is the URL itself. Every change is mirrored into the page URL, and you can also export a JSON backup.
          For external live embedding, use the iframe embed link below. A true always-updating public PNG URL is not possible on plain
          static hosting without a backend, so PNG export is provided as a snapshot.
        </div>
      )}
      ...
      {!embedMode && (
        <>
          <div className="actions">
            ...
            <button onClick={onCopyEmbedLink} disabled={!tournament}>Copy iframe embed link</button>
        ...
        <>
          ...
          {!embedMode && (
            <div className="actions secondary-actions">
              ...
              <button onClick={onDownloadPng} disabled={!pngUrl}>Download PNG snapshot</button>
              <button onClick={onCopyPng} disabled={!pngUrl}>Copy PNG data URL</button>
              <span>{pngMsg}</span>
            </div>
          )}
